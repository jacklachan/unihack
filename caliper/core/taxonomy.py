...
RULES: Tuple[Tuple[str, TaxonomyNode], ...] = (
    ...
    # ---- hand tools ------------------------------------------------------
    (r"\b(screwdriver|nut\s+driver|hex\s+key|allen\s+key)\b",
     TaxonomyNode("Tools", "Hand Tools", "Screwdrivers",
                  "Tools & Equipment>Hand Tools>Screwdrivers", "27111500")),
    (r"\b(pliers|snips|shears|nipper)\b",
     TaxonomyNode("Tools", "Hand Tools", "Pliers & Cutters",
                  "Tools & Equipment>Hand Tools>Pliers & Cutters", "27111600")),
    (r"\b(hammer|mallet|sledge)\b",
     TaxonomyNode("Tools", "Hand Tools", "Hammers",
                  "Tools & Equipment>Hand Tools>Hammers & Mallets", "27111900")),
    (r"\b(chisel|punch|pry\s*bar|scraper)\b",
     TaxonomyNode("Tools", "Hand Tools", "Striking & Struck Tools",
                  "Tools & Equipment>Hand Tools>Chisels & Punches", "27111900")),
    (r"\b(clamp|vise|vice)\b",
     TaxonomyNode("Tools", "Workholding", "Clamps & Vises",
                  "Tools & Equipment>Workholding>Clamps & Vises", "27112400")),
    (r"\b(file|rasp)\b",
     TaxonomyNode("Tools", "Hand Tools", "Files",
                  "Tools & Equipment>Hand Tools>Files & Rasps", "27111900")),
    (r"\b(work\s*bench|saw\s*horse|table\s+assembly|stock\s+feeder)\b",
     TaxonomyNode("Tools", "Shop Equipment", "Benches & Supports",
                  "Tools & Equipment>Shop Equipment>Benches & Supports", "56101700")),
    (r"\b(ladder|step\s*stool|scaffold)\b",
     TaxonomyNode("Tools", "Access Equipment", "Ladders",
                  "Tools & Equipment>Access Equipment>Ladders", "27112700")),
    (r"\b(tool\s*bag|tool\s*belt|holster|pouch)\b",
     TaxonomyNode("Tools", "Storage", "Tool Bags & Holsters",
                  "Tools & Equipment>Tool Storage>Bags & Pouches", "24101600")),
    (r"\b(dust\s+collector|air\s+hose|compressor)\b",
     TaxonomyNode("Tools", "Shop Equipment", "Air & Dust Management",
                  "Tools & Equipment>Shop Equipment>Air & Dust Management", "40151500")),
    (r"\b(pressure\s+gauge|gauge)\b",
     TaxonomyNode("Tools", "Instruments", "Gauges",
                  "Tools & Equipment>Instruments>Gauges", "41111600")),

    # ---- safety ----------------------------------------------------------
    (r"\b(respirator|dust\s+mask)\b",
     TaxonomyNode("Safety", "PPE", "Respiratory Protection",
                  "Safety>Personal Protective Equipment>Respiratory Protection", "46182000")),
    (r"\b(hard\s*hat|helmet)\b",
     TaxonomyNode("Safety", "PPE", "Head Protection",
                  "Safety>Personal Protective Equipment>Head Protection", "46181700")),
    (r"\b(ear\s*muff|ear\s*plug)\b",
     TaxonomyNode("Safety", "PPE", "Hearing Protection",
                  "Safety>Personal Protective Equipment>Eye & Ear Protection", "46181800")),
    (r"\b(knee\s*pad)\b",
     TaxonomyNode("Safety", "PPE", "Knee Protection",
                  "Safety>Personal Protective Equipment>Knee Protection", "46181500")),

    # ---- electrical accessories -----------------------------------------
    (r"\b(power\s*strip|surge\s*protector|wall\s*tap|adapter\s*plug)\b",
     TaxonomyNode("Electrical", "Wiring Devices", "Power Strips & Taps",
                  "Electrical>Wiring Devices>Power Strips & Surge Protectors", "39121300")),
    (r"\b(junction\s+box|outlet\s+box)\b",
     TaxonomyNode("Electrical", "Boxes & Enclosures", "Junction Boxes",
                  "Electrical>Boxes & Enclosures>Junction Boxes", "39131700")),
    (r"\b(water\s+heater)\b",
     TaxonomyNode("Plumbing", "Water Heaters", "Water Heaters",
                  "Plumbing>Water Heaters>Water Heaters", "40101800")),
    (r"\b(garbage\s+disposal|disposer)\b",
     TaxonomyNode("Appliances", "Kitchen", "Garbage Disposals",
                  "Appliances & Consumer Electronics>Kitchen Appliances>Garbage Disposals",
                  "52141600")),
    (r"\b(soffit)\b",
     TaxonomyNode("Building Materials", "Trim", "Soffit",
                  "Building Materials>Trim & Moulding>Soffit", "30103600")),

    # ---- openings, envelope and exterior --------------------------------
    (r"\b(skylight|sun\s*tunnel|roof\s*window)\b",
     TaxonomyNode("Building Materials", "Windows & Doors", "Skylights",
                  "Building Materials>Windows & Doors>Skylights", "30171500")),
    (r"\b(patio\s+door|gliding\s+door|sliding\s+door|entry\s+door|"
     r"storm\s+door|access\s+door|door)\b",
     TaxonomyNode("Building Materials", "Windows & Doors", "Doors",
                  "Building Materials>Windows & Doors>Doors", "30171500")),
    (r"\b(casement|awning\s+window|hopper|basement\s+window|window)\b",
     TaxonomyNode("Building Materials", "Windows & Doors", "Windows",
                  "Building Materials>Windows & Doors>Windows", "30171600")),
    (r"\b(baluster|balusters)\b",
     TaxonomyNode("Building Materials", "Decking", "Balusters",
                  "Building Materials>Decking>Railing Systems", "30103600")),
    (r"\b(gate)\b",
     TaxonomyNode("Building Materials", "Decking", "Gates",
                  "Building Materials>Decking>Gates", "30103600")),
    (r"\b(rib|roof\s*panel|metal\s*panel|standing\s+seam|corrugated)\b",
     TaxonomyNode("Building Materials", "Roofing", "Metal Roofing Panels",
                  "Building Materials>Roofing>Metal Roofing Panels", "30151600")),
    (r"\b(shingle|underlayment|ice\s*guard|drip\s*edge|flashing)\b",
     TaxonomyNode("Building Materials", "Roofing", "Roofing Accessories",
                  "Building Materials>Roofing>Roofing Accessories", "30151500")),
    (r"\b(insulation|house\s*wrap|vapor\s*barrier|sheathing)\b",
     TaxonomyNode("Building Materials", "Insulation", "Insulation",
                  "Building Materials>Insulation>Insulation", "30141500")),
    (r"\b(sanding\s+sponge|sponge)\b",
     TaxonomyNode("Tools", "Power Tool Accessories", "Sanding Sponges",
                  "Tools & Equipment>Abrasives>Sanding Sponges", "31191500")),
    (r"\b(saw)\b",
     TaxonomyNode("Tools", "Power Tools", "Saws",
                  "Tools & Equipment>Power Tools>Saws", "27112100")),
    (r"\b(impact)\b",
